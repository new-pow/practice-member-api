...
public class JwtService {
    ...
    public Token getToken(Long memberId) throws IOException {
        ...
        Token.AccessToken accessToken = jwtProvider.generateAccessToken(new Logined(memberId), now);
        Token.RefreshToken refreshToken = jwtProvider.generateRefreshToken(new Logined(memberId), now);
    ...
    @Transactional
    public Token refreshAccessToken(Member member, Refresh.Request request) throws IOException {
        Token token = authRepository.findByIdAndRefreshToken(member.getId(), request.getRefreshToken())
        ...
        Token.AccessToken accessToken = jwtProvider.refreshAccessToken(member.getId(), token.getRefreshTokenValue());
